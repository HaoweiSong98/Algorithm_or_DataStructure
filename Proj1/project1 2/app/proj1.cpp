...
bool puzzleSolver(std::string s1, std::string s2, std::string s3, std::map<char, unsigned> & mapping)
{
	...
	int result[10];
	vector<int> check_sheet;
	for(int i = 0; i<10;i++){
		check_sheet.push_back(0);
	}
	vector<string> total;
...
void all_result(int start_i, int num_char, int all_num[10],vector<string> & total, int result[],vector<int> check_sheet)
{
...
void changeSheet(int sheet_in, vector<int> & check_sheet, int step_in){
	check_sheet[sheet_in] += step_in;
}
