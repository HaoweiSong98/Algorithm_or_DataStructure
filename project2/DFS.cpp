...
//	g1[7].push_back(6);
	...
	find_path(g1,0,1,check_sheet,the_path,total_result);
	//find_path(g1,0,2,check_sheet,the_path,total_result);
	//find_path(g1,0,3,check_sheet,the_path,total_result);
	//cout<<total_result[1].size()<<endl;
	for(int i=0;i<total_result.size();i++){
		for(int j = 0;i<total_result[i].size();j++){
			cout<<total_result[i][j];
		}
	}
		//find_path(g1, 0, 3, check_sheet, the_path, total_result);
